...
#   --no-cron       Skip cron jobs (also skips NIC guard install)
...
Flags:
  ...
  --no-cron       Skip cron installation (also skips NIC guard install)
...
refresh_network_stack() {
  ...
  if [ $tried -eq 0 ]; then
    add_summary "Network refresh skipped (no known network services: NetworkManager/systemd-networkd/networking)"
  elif [ $ok -eq 0 ]; then
    add_summary "Network refresh attempted but no restart succeeded"
  fi
}

# ---------------- NIC guard installer (post-boot via systemd) ----------------
install_nic_guard() {
  if $NO_CRON; then
...
#   - If both unreachable -> try restarting:
#         NetworkManager, then systemd-networkd, then networking.service
#
...
bounce_network_stack() {
  if ! command -v systemctl >/dev/null 2>&1; then
    echo "systemctl not available; cannot manage network services."
    return 1
  fi
  ...
  if [ $tried -eq 0 ]; then
    echo "No known network services found to restart."
    return 1
  fi
...
echo "Both gateway and 8.8.8.8 unreachable. Attempting network stack restart..."

if bounce_network_stack; then
  echo "Network stack restart completed. Re-checking gateway ping..."
  sleep 5
  if ping -c 3 -W 2 "$GW" >/dev/null 2>&1; then
  ...
  echo "No network service could be restarted or restart failed."
fi

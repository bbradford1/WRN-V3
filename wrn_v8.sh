...
#   - NetworkManager post-boot refresh (nmcli + netplan apply) w/ logs
#
# Flags:
...
Default:
  • fstab hardening (safe; loop devices only, with backup)
  ...
  • NM post-boot refresh (nmcli reapply + netplan apply) with logs
...
# ---------------- NetworkManager / NM refresh installer ----------------
install_nm_boot_fix() {
  if $NO_CRON; then
  ...
  # Optional: delay NetworkManager start if the unit exists
  if systemctl list-unit-files 2>/dev/null | awk '{print $1}' | grep -qx "NetworkManager.service"; then
...
EOF
  ...
  # Helper script that performs NM/netplan refresh + logs
...
if command -v nmcli >/dev/null 2>&1; then
  ...
  echo "Running nmcli device reapply on all ethernet devices..."
  for dev in $(nmcli -t -f DEVICE,TYPE device status | awk -F: '$2=="ethernet"{print $1}'); do
    echo "  - reapply on $dev..."
    nmcli device reapply "$dev" || echo "    nmcli device reapply failed on $dev"
  done
...
REF
  ...
  # Systemd unit that runs this once shortly after boot
  local UNIT_FILE="/etc/systemd/system/wrn-nm-refresh.service"
  cat > "$UNIT_FILE" <<'UNIT'
[Unit]
...
UNIT
  ...
  add_summary "NM boot-fix installed: wrn_nm_refresh.sh + wrn-nm-refresh.service (post-boot nm/netplan refresh + logs to Downloads)"
}
...
install_cron() {
  ...
  cp -f "$0" /root/wrn_v8.sh
  chmod 755 /root/wrn_v8.sh

  tmp="$(mktemp)"
  crontab -l 2>/dev/null | grep -v 'wrn_v8.sh' > "$tmp" || true

  echo '12 3 * * 1 /usr/bin/bash /root/wrn_v8.sh >> /var/log/system_cleanup.log 2>&1' >> "$tmp"
  echo '30 3 1 * * /usr/bin/bash /root/wrn_v8.sh --journal-only >> /var/log/system_cleanup.log 2>&1' >> "$tmp"

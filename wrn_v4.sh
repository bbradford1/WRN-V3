#!/bin/bash
#
# Script: wrn_v4.sh  (standalone, test-ready)  -- PATCHED VALIDATION
# Purpose: WRN boot fix & cleanup + PERSISTENT data-disk mounts via /etc/fstab (UUID)
# Author: Bradford & Troy — v4 test build
# Date: 2025-11-02
#
# Changes in this patch:
#  - Replaced `systemd-analyze verify /etc/fstab` (not portable) with a robust validation:
#      1) If available:   findmnt --verify --tab-file /etc/fstab
#      2) Else fallback:   mount -fav   (fake/verbose mount; parses fstab without changing state)
#
set -euo pipefail
...
if [[ "$valid_ok" -eq 0 ]]; then
  echo "[note] Falling back to 'mount -fav' for fstab validation…"
  if mount -fav; then
    valid_ok=1
  else
    valid_ok=0
...
if [[ "$valid_ok" -ne 1 ]]; then
  echo "[ERROR] fstab validation failed. Restoring backup and aborting."
